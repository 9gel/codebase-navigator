...
**An absent symbol may be an absent dependency.** When a repository-wide search finds nothing, `grep_search` checks the project's manifests (`package.json`, `pyproject.toml`, `Cargo.toml`, `go.mod`, `requirements.txt`, …) and names any declared dependency the pattern refers to. Express 5 moved routing into the external `router` package, so an agent asked where the router matches layers found nothing in `lib/` and spent **11 of its 24 turns** running `ls` and `find` over a repository that never contained it (−113.5% against the baseline). The same shape had appeared earlier and cheaper on `express-json-body-parser`, hunting `node_modules/body-parser`.

**Tool payloads carry repository-relative paths.** An absolute `file://` URI on every hit was **50% of a grep result** (562 of 1,120 tokens on one fastapi search) and **26% of the seed** — and both are re-sent on every subsequent turn. The URIs exist so the final answer can carry clickable links, so the repository root is now stated once in the first message and tool results show `path:line` only. Measured on `fastapi-dependency-overrides`: grep 1,120 → 538 tokens, seed 1,048 → 760.
